...
export default function Chatbot() {
  ...
  // NOTE: removed the old auto-open-after-5-seconds behavior. The chat
  // now only opens when the visitor actually taps the bubble or button —
  // it was previously forcing itself open on every visitor automatically,
  // which is what made it feel like it "took over the screen."
    ...
    <>
      ...
      {open && (
        <div style={{
          ...
          <div style={{ background: G, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
            ...
            <button onClick={() => setOpen(false)} style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", width: 24, height: 24, borderRadius: "50%", cursor: "pointer", fontSize: 14, flexShrink: 0 }}>×</button>
          </div>
          {step === "form" ? (
            <div style={{ padding: 20, background: "#f7f9fc", overflowY: "auto" }}>
            ...
            <>
              <div style={{ flex: 1, minHeight: 200, maxHeight: 320, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: 10, background: "#f7f9fc" }}>
                {msgs.map((m, i) => (
              ...
              <div style={{ padding: "6px 12px", background: "#e9fdf1", borderTop: "1px solid #b2f0ce", display: "flex", justifyContent: "center", flexShrink: 0 }}>
              ...
              <div style={{ padding: "9px 12px", background: "#fff", borderTop: "1px solid #eee", display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
                ...
                <button onClick={() => send()} disabled={!input.trim()||loading} style={{ width: 36, height: 36, borderRadius: "50%", background: input.trim()&&!loading ? G : "#eee", border: "none", cursor: "pointer", flexShrink: 0 }}>➤</button>
              </div>
            </>
          )}
        </div>
