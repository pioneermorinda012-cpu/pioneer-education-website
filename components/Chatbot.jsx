      ...
      <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 1000, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 10 }}>
        {bubble && !open && (
          <div style={{ background: "#fff", border: "1px solid #e0e0e0", borderRadius: "16px 16px 4px 16px", padding: "10px 14px", fontSize: 13, color: "#222", boxShadow: "0 4px 20px rgba(0,0,0,0.12)", maxWidth: 200, cursor: "pointer", position: "relative" }} onClick={() => { setOpen(true); setBubble(false); }}>
            ...
            <button onClick={e => { e.stopPropagation(); setBubble(false); }} style={{ position: "absolute", top: -6, right: -6, width: 18, height: 18, borderRadius: "50%", background: "#999", border: "none", color: "#fff", fontSize: 10, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>?</button>
          </div>
        )}
        <button onClick={() => { setOpen(!open); setBubble(false); }}
          style={{ width: 60, height: 60, borderRadius: "50%", background: G, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, boxShadow: "0 4px 20px rgba(26,107,69,0.4)", transition: "transform 0.2s" }}>
          {open ? "?" : "??"}
        </button>
      </div>
